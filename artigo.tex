...
\author{
{\large 邱俊明 3230102363}\\ {\large 刘烨 3230105721}\\  {\large 李文耀 3230102302}\\   
}
\criartitulo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsubsection{对自适应与裁剪类优化算法的理论启发}

文章对 GD、NGD 与 Clip-GD 统一刻画了“先线性、后次线性”的收敛结构， 为进一步分析自适应步长方法或带裁剪机制的优化算法（如参数自适应策略）提供了理论启发。

\subsubsection{在弱反馈与低精度优化中的研究前景}

文章证明了在仅使用坐标梯度甚至函数值比较的情况下仍可获得较快的收敛阶段， 为黑盒优化、弱监督学习及低精度计算场景中的优化方法研究提供了新的理论方向。
